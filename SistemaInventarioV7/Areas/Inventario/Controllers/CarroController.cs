...
namespace SistemaInventarioV7.Areas.Inventario.Controllers
{
    ...
    public class CarroController : Controller
    {
        ...
        public async Task<IActionResult> menos(int carroId)
        {
            ...
            {
                //Removemos el registro del carro de compras y actualizamos la sesión 
                var carroLista = await _unidadTrabajo.CarroCompra.ObtenerTodos
                                                        (c => c.UsuarioAplicacionId == carroCompras.UsuarioAplicacionId);
                var numeroDeProductos = carroLista.Count();
                _unidadTrabajo.CarroCompra.Remover(carroCompras);
                await _unidadTrabajo.Guardar();
                HttpContext.Session.SetInt32(DS.ssCarroCompras, numeroDeProductos - 1);
            }
            else
            {
                carroCompras.Cantidad -= 1;
        ...
        public async Task<IActionResult> Proceder()
        {
            ...
            carroCompraVM.Orden.TotalOrden = 0;
            carroCompraVM.Orden.UsuarioAplicacion = await _unidadTrabajo.UsuarioAplicacion
                                                                .ObtenerPrimero(u => u.Id == claim.Value);
            ...
            foreach (var lista in carroCompraVM.CarroCompraLista)
            {
                var producto = await _unidadTrabajo.BodegaProducto
                            .ObtenerPrimero(b => b.ProductoId == lista.Producto.Id &&
        ...
        public async Task<IActionResult> OrdenConfirmacion(int id)
        {
            ...
            Console.WriteLine("Probando esto igual: " + orden);
            var service = new SessionService();
            Console.WriteLine("probando esto: " + orden.SessionId);
            ...
            if (session.PaymentStatus.ToLower() == "paid")
            {
                ...
                //Disminuir stock de la bodega venta
                ...
                foreach(var lista in carroCompra)
                {
                    ...
                    bodegaProducto = await _unidadTrabajo.BodegaProducto
                                        .ObtenerPrimero(b => b.ProductoId == lista.ProductoId &&
            ...
            //Borramos el carro de compra y la sesion del carro de compras
